...
//for a grid tht is x[m][n]
    ...
    int n=m;
//this is a square :)
// create m by n matrix- note this needs to be a double as we are looking at probabilities- could change if this is too slow
...
//assume starting position is in the middle bottom
    ...
    {
    //what cell is the tank in, note that the x is forward and y is sideways, 0,0 is defined to be the bottom right corner -return a x and y for what cell we are looking at stuff in
    ...
    //startign closest to the tank, look at all the grid squares in the way by converting these points to a straight line
    // y=mx+b for now we are using Bresham's algorithim
        std::vector<std::pair<double, double>> cell_list = breshams_alg(i, sen_hori, sen_vert, w_Tank_Position)
            while (!cell_list.empty()) {   //this will give a list of grid ms and ns that need to be looked at, starting from closest to the tank
                double cell_dist = ???;
                probability(cell_list->first, cell_list->second, cell_dist, gen_sonics[i]);     //function to calculate probability
