...
double w_Tank_Position[2] = {0};
const int start_row = 6;
const int start_column = 6;

void breshams_alg(double object_cell_m, double object_cell_n,
                  double tank_cell_m, double tank_cell_n) {
  ...
  double e = 0;
  double y = tank_cell_m;

  for (double x = tank_cell_n; x <= object_cell_n; x++) {
    // Grid::cell_list.emplace_back(std::make_pair(
    //     std::floor(x / Grid::gridspace), std::floor(y / Grid::gridspace)));
    e += de;

    std::cout << "Position " << x << " of " << object_cell_n << std::endl;
    std::cout << "Cell " << x << " " << y << std::endl;

    if (e >= 0.5) {
      y += (std::signbit(dy) == 1 ? -1 : 1);
      e -= gridspace;
    }
  }
}

void dda(double object_cell_m, double object_cell_n, double tank_cell_m,
         double tank_cell_n) {
  double x;
  double y;
...
void nonbresh(double object_cell_m, double object_cell_n, double tank_cell_m,
              double tank_cell_n) {
  ...
  // ahead and dx/dy=infinity
    ...
    // for (double x = tank_cell_n; x <= object_cell_n; inc) {
    //   xval = tank_cell_n + multiplier * inc;
    //   yval = (1 / inc) * xval + object_cell_n - (1 / inc) * object_cell_m;
    //   std::cout << "x val " << xval << "\ty val " << yval << std::endl;
    //   // Grid::cell_list.emplace_back(
    //   // std::make_pair(std::floor(xval), std::floor(yval)));

    //   multiplier++;
    // }
    // breshams_alg(object_cell_m, object_cell_n, tank_cell_m, tank_cell_n);
