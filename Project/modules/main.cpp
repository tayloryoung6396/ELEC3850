...
int main() {
    ...
    while (1) {
        ...
        data2  = 100;
        result = executeWriteSingle(servo_ID, MX28_ADDRESS_VALUE(VELOCITY_P_GAIN), reinterpret_cast<uint8_t*>(&data2));
        std::cout << "V_P Gain Result " << result << std::endl;
        data2  = 1920;
        result = executeWriteSingle(servo_ID, MX28_ADDRESS_VALUE(VELOCITY_I_GAIN), reinterpret_cast<uint8_t*>(&data2));
        std::cout << "V_I Gain Result " << result << std::endl;
        uint32_t data3 = 0;
        ...
        data3  = 50;
        result = executeWriteSingle(servo_ID, MX28_ADDRESS_VALUE(GOAL_VELOCITY), reinterpret_cast<uint8_t*>(&data3));
        std::cout << "VEL Goal Result " << result << std::endl;
        // For each iteration
        // Check sensors
        Camera_main();
        InfraredSensor_main();
