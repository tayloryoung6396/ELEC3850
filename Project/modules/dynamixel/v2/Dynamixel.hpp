...
namespace dynamixel {
namespace v2 {
    // PING          Instruction that checks whether the Packet has arrived to a device with the same ID as Packet ID
    // READ          Instruction to read data from the Device
    // WRITE         Instruction to write data on the Device
    // REG_WRITE     Instruction that registers the Instruction Packet to a standby status; Packet is later executed
    //               through the Action command
    // ACTION        Instruction that executes the Packet that was registered beforehand using Reg Write
    // FACTORY_RESET Instruction that resets the Control Table to its initial factory default settings
    // REBOOT        Instruction to reboot the Device
    // STATUS_RETURN Return Instruction for the Instruction Packet
    // SYNC_READ     For multiple devices, Instruction to read data from the same Address with the same length
    //               at once
    // SYNC_WRITE    For multiple devices, Instruction to write data on the same Address with the same length
    //               at once
    // BULK_READ     For multiple devices, Instruction to read data from different Addresses with different
    //               lengths at once
    // BULK_WRITE    For multiple devices, Instruction to write data on different Addresses with different
    //               lengths at once
    ...
    inline uint16_t calculateChecksum(const T* packet, uint16_t crc_accum = 0) {
        ...
        size_t packet_len = sizeof(T) - sizeof(packet->checksum);
