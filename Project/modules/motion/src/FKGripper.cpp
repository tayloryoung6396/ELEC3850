...
double* FK_Calculate(double base_yaw, double base_pitch, double elbow_pitch, double wrist_pitch) {
    ...
    double Pres_pos[3]          = {0};
    double Pres_pos[2]          = std::sin(elevation_base_pitch - theta_base_pitch) * arm_len_3;
    double arm_len_xy           = std::cos(elevation_base_pitch - theta_base_pitch) * arm_len_3;
    double Pres_pos[1]          = std::sin(base_yaw) * arm_len_xy;
    double Pres_pos[0]          = std::cos(base_yaw) * arm_len_xy;
